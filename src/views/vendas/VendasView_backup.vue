...
const loadLotesForEvent = async () => {
  ...
  try {
    ...
    toastStore.addToast({
      type: 'error',
      message: 'Erro ao carregar lotes do evento',
      duration: 3000
    })
  } finally {
...
const addLoteToCart = (lote: Lote) => {
  ...
  if (existingItem) {
    ...
    // Iniciar timer de reserva se for o primeiro item
...
const removeLoteFromCart = (loteId: number) => {
  ...
  if (itemIndex !== -1) {
    ...
    // Parar timer se carrinho ficar vazio
...
const startReservationTimer = () => {
  if (reservationTimer.value) return
  
  reservationExpiry.value = new Date(Date.now() + 10 * 60 * 1000) // 10 minutos
  reservationTimer.value = window.setInterval(() => {
    if (reservationExpiry.value && Date.now() >= reservationExpiry.value.getTime()) {
      clearCart()
      stopReservationTimer()
      toastStore.addToast({
        type: 'warning',
        message: 'Reserva expirada. Carrinho foi limpo.',
        duration: 5000
      })
    }
  }, 1000)
...
const processCheckout = async () => {
  ...
  try {
    ...
    toastStore.addToast({
      type: 'success',
      message: `Venda realizada com sucesso! Pedido #${checkoutResult.value?.pedidoId}`,
      duration: 5000
    })
